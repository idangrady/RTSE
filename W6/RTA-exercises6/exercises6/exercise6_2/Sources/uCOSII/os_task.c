...
#if OS_LOG_EN > 0
        if (err == OS_ERR_NONE) {
          logTaskCreated(OSTCBPrioTbl[prio]);
        }
#endif
        if (err == OS_ERR_NONE) {
...
/*$PAGE*/
    ...
    RelteqServerMakeTaskNotReady(ptcb);
#endif
...
/*
*********************************************************************************************************
* Description: This funstion is used to assign a preemption threshold to a task.
* Arguments:   prio:      the task for which the threshold is given
*              threshold: the threshold value, which must be a valid task priority
*                       (Note that threshold == 0 => non-preemptive scheduling of tasks
* Returns :    none
*********************************************************************************************************
*/
void OSTaskThresholdSet(INT8U prio, INT8U threshold)
{
  OSTCBPrioTbl[prio]->OSTCBThreshold = threshold;
}

/*
*********************************************************************************************************
* Description: This funstion is used to query the preemption threshold of a task.
* Arguments:   prio:      the task for which the threshold is returned
* Returns :    the task's threshold value
*********************************************************************************************************
*/
INT8U OSTaskThresholdGet      (INT8U            prio)
{
  return OSTCBPrioTbl[prio]->OSTCBThreshold;
}
