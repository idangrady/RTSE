...
void  OSInit (void)
{
    ...
    OS_MutexSRPInit();                                           /* Initialize the SRP datastructures        */
#endif
...
#if OS_RELTEQ_EN > 0
                /* check if exiting the timer ISR without the need to schedule */
                if (!RelteqNeedScheduling) {
                        ...
                        INT8U      pend_stat)
{
    ...
    if ((ptcb->OSTCBStat &   OS_STAT_SUSPEND) == OS_STAT_RDY) {
        ...
        RelteqServerMakeTaskReady(ptcb);
#endif
    }
...
static  void  OS_SchedNew (void)
{
  ...
  /* The highest priority ready task within this server must have a higher preemption threshold than the subsystem ceiling */
  /* Otherwise we return the task on top of the resource stack of the currently scheduled server                           */
  INT8U sprio = OSPrioServHighRdy;
  if ( OSSCBTbl[sprio].OSSRPLocalTopOfStack != OSSCBTbl[sprio].OSSRPServerCeiling 
      && OSSCBTbl[sprio].OSSRPLocalTopOfStack->localCeiling <= OSTCBPrioTbl[OSPrioHighRdy]->OSTCBThreshold )
  {
    OSPrioHighRdy = OSSCBTbl[sprio].OSSRPLocalTopOfStack->localLockingTask;
    if (OSTCBPrioTbl[OSPrioHighRdy]->OSTCBStat != OS_STAT_RDY) { 
      /* In case of self-suspension inside a critical section, the idle task is switched-in */
      if (OSSCBTbl[sprio].OSServerType == OS_TEV_SERVER_TYPE_PERIODIC_IDLING) {
        OSPrioHighRdy = OS_SERVER_PRIO(sprio, OS_TASK_SERVER_IDLE_PRIO);
      } else {
...
#else
  /* In case of SRP, highest priority ready task must have a higher preemption threshold than the system ceiling */
  /* Otherwise we return the task on top of the resource stack                                                   */
  if ( OSSRPTopOfStack != OSSRPSystemCeiling && OSSRPTopOfStack->ceiling <= OSTCBPrioTbl[OSPrioHighRdy]->OSTCBThreshold )
  {
    OSPrioHighRdy = OSSRPTopOfStack->lockingTask;
    if (OSTCBPrioTbl[OSPrioHighRdy]->OSTCBStat != OS_STAT_RDY) {
        OSPrioHighRdy =  OS_TASK_IDLE_PRIO;
    }
  }
...
#if OS_MUTEX_SRP_EN > 0
    OSTaskThresholdSet( OS_PRIO_SELF,  0 );      /* Idle task has highest preemption threshold!        */
#endif
    for (;;) {
        OS_ENTER_CRITICAL();
        OSIdleCtr++;
...
#if OS_MUTEX_SRP_EN > 0                              /* Load task preemption threshold into TCB  */
        ptcb->OSTCBThreshold     = prio;
#endif
...
#if OS_RELTEQ_EN > 0
        ptcb->OSTCBPeriod            = 0;
        ptcb->OSTCBPeriodEvent       = (OS_EVENT*)0;
        ptcb->OSTCBPeriodRelteqEvent = (RelteqEvent*)0;
#if OS_RELTEQ_SPORADIC_TASK_EN > 0
        ptcb->OSTCBSporadicFlag      = 0;
#endif
#if OS_RELTEQ_HSF_EN > 0   
        ptcb->OSTCBServer            = (RelteqServer*)0;
#endif
#endif /* OS_RELTEQ_EN > 0 */
